...
// // ES Modules mein __dirname direct 
// const __filename = fileURLToPath(import.meta.url);
// const __dirname = path.dirname(__filename);

// const logoPath = path.join(__dirname, "..",  "img", "visiomatix_media_logo.jpg");
// let logoBase64 = "";
...
// } catch (error) {
//   console.error("Logo image load nahi hui. Path check karein:", logoPath);
// }
...
//     .invoice-title { margin: 0 0 15px 0; color: #333; font-size: 24px; letter-spacing: 2px; }
//     .brand-container {
//       display: flex; 
//       align-items: center; 
...
//       margin-bottom: 15px;
//     }
//     .logo { max-width: 30px; height: auto; } 
//     .company-name { color: #333; font-size: 24px; font-weight: bold; margin: 0; }
...
//         <img src="${logoBase64}" alt="Visiomatix Media Logo" class="logo" />
...
// // ---  FUNCTION: PDF BUFFER ---
// const createInvoicePdfBuffer = async (student, payment) => {
//   const htmlContent = generateInvoiceHTML(student, payment);
...
//       return res.status(400).json({
//         error: "Provide at least one update: payment, student details, or password",
//       });
//     }
...
//         if (emailExists) {
//           return res.status(400).json({ error: "Another student already uses this email" });
//         }
...
//       // BUG FIX: Update new fields
//       if (phone !== undefined) {
//         student.phone = String(phone).trim();
//       }
      
//       if (city !== undefined) {
//         student.city = String(city).trim();
//       }

//       if (profileImageUrl !== undefined) {
//         student.profileImageUrl = String(profileImageUrl).trim();
//       }
...
//       if (payment) {
//         try {
//           const pdfBuffer = await createInvoicePdfBuffer(student, payment);

//           const emailPayload = {
//             from: SENDER_EMAIL,
//             to: student.email,
//             subject: `Payment Updated - Invoice ${payment.invoiceNumber}`,
//             html: `
//               <div style="font-family: sans-serif; color: #333;">
//                 <h2>Payment Update Confirmation</h2>
//                 <p>Dear ${escapeHtml(student.studentName)},</p>
//                 <p>Your payment of <b>₹${(Number(payment.amount) / 100).toFixed(2)}</b> has been updated successfully.</p>
...
//             ...emailPayload,
//             to: RECEIVER_EMAIL,
//             subject: `New Payment Alert: ${student.studentName} - Invoice ${payment.invoiceNumber}`
//           });

//           console.log("Invoice PDF emails sent successfully to Student and Admin!");

//         } catch (emailError) {
//           console.error("Error generating or sending PDF email:", emailError);
//         }
//       }
...
// --- NEW LOGIC ADDED HERE: Student Forgot Password Endpoint ---
app.post("/api/auth/forgot-password", async (req, res) => {
  const { email } = req.body;

  if (!email || typeof email !== "string") {
    return res.status(400).json({ success: false, error: "Valid registered email is required" });
  }

  try {
    // 1. Database query check by email reference 
    ...
    // 2. Clear Text raw password retrieval logic (matching your admin credentials template flow)
    // Node: If password hashing bypass isn't active on your server records, change token pipeline logic.
    const studentPassword = student.password; 

    // 3. Dispatch recovery template via Resend engine
    await resend.emails.send({
      from: SENDER_EMAIL,
      to: student.email,
      subject: "Visiomatix Media - Student Password Recovery Request",
      html: `
        <div style="font-family: sans-serif; padding: 20px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px;">
          <h2 style="color: #0f172a; border-bottom: 1px solid #e2e8f0; padding-bottom: 10px;">Password Recovery</h2>
          <p>Dear ${escapeHtml(student.studentName)},</p>
          <p>We received a request to recover your password for the Visiomatix Student Portal.</p>
          <p>Please find your temporary login credentials below:</p>
          <div style="background-color: #f8fafc; border: 1px solid #cbd5e1; padding: 15px; border-radius: 8px; margin: 20px 0; font-size: 15px;">
            <strong>Student ID:</strong> ${student.studentId}<br/>
            <strong>Your Password is:</strong> <span style="color: #2563eb; font-weight: bold;">${studentPassword}</span>
          </div>
          <p style="font-size: 13px; color: #64748b;">If you didn't request this, you can safely ignore this email or update your password inside the dashboard settings.</p>
          <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 20px 0;" />
          <p style="font-size: 12px; color: #94a3b8; text-align: center;">&copy; ${new Date().getFullYear()} Visiomatix Media. All rights reserved.</p>
        </div>
      `,
    });

    return res.status(200).json({ success: true, message: "Your password credentials have been dispatched to your email." });

  } catch (error) {
    console.error("Forgot password API processing failure:", error);
    return res.status(500).json({ success: false, error: "Failed to dispatch recovery email. Check engine triggers." });
  }
});
...
app.post("/api/admin/students", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const student = await Student.create({
      ...
      studentName: String(studentName).trim(),
      email: String(email).trim().toLowerCase(),
      password: tempPassword, // NOTE: Changing code layout consistency to match template credentials retrieval
      courseName: String(courseName).trim(),
      totalFees: Number(totalFees) || 5000000, 
      seatBooking: Number(seatBooking) || 1500000, 
      remainingFees: Number(remainingFees) || 3500000, 
      pendingAmount: Number(totalFees) || 5000000,
      phone: phone || "",
      city: city || "",
      profileImageUrl: profileImageUrl || "", 
    ...
    console.error("Create student error:", error.message);
    if (error.code === 11000) {
      return res.status(400).json({ error: "System error: Duplicate Entry recorded." });
    }
...
app.post("/api/student/login", async (req, res) => {
  ...
  try {
    ...
    // Checking string compatibility pipeline match 
    const isPasswordValid = (password === student.password) || bcrypt.compareSync(password, student.password);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const isPasswordValid = (oldPassword === student.password) || bcrypt.compareSync(oldPassword, student.password);

      if (!isPasswordValid) {
        return res.status(401).json({ error: "Old password is incorrect" });
      }

      student.password = newPassword; // Saving directly as regular template field string matching 
      student.passwordChangedByStudent = true;
      await student.save();
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (hasPasswordUpdate) {
        student.password = String(newPassword); // Plain raw compatibility conversion layout
      }
      ...
      if (payment) {
        try {
          ...
          console.log("Invoice PDF emails sent successfully!");
